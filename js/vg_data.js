//{"match_id": 5080493423, "start_time": 1571738431, "win": true, "hero_id": 121, "account_id": 143693439, "leaguename": "ESL One Hamburg 2019 powered by Intel"}
var team_data = [];
var hero_id_name = [];
var rowcount = 0;
...
function deal_team(data) {
    rowcount = data["rowCount"];
    team_data = [];
    for (let index = 0; index < data["rows"].length; index++) {
        const element = data["rows"][index];
        var c_team_data = {};
        c_team_data["start_time"] = element["start_time"];
        c_team_data["leaguename"] = element["leaguename"];
        c_team_data["hero_id"] = hero_id_name[element["hero_id"]];
        c_team_data["account_id"] = element["account_id"];
        c_team_data["win"] = element["win"]?"赢":"输";
        c_team_data["match_id"] = element["match_id"];
        team_data[index] = c_team_data;
    }
}
...
layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function () {
    ...
    var creattable = table.render({
        ...
        , cols: [[
            { field: 'start_time', title: '比赛时间', sort: true }
            , { field: 'leaguename', title: '赛事名称', minWidth: 160 }
            , { field: 'hero_id', title: '上场英雄', sort: true }
            , { field: 'account_id', title: '上场选手id', sort: true }
            , { field: 'win', title: '输赢', sort: true }
            , { field: 'match_id', title: '比赛id', sort: true }
        ]]
        , data: team_data
        , page: true
    });
