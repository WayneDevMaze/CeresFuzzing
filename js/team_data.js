...
function floatDiv(arg1, arg2) {
    ...
    var f_result = (r1 / r2) * Math.pow(10, t2 - t1);
    return f_result.toFixed(3);
}
function calculate(c_data,c_hero_id) {
    // c_data数组，每个元素是json
    var count = 0, win_count = 0;
    for (let index = 0; index < c_data.length; index++) {
        const element = c_data[index];
        if (element["hero_id"] == c_hero_id) {
            count += 1;
            if (element["win"] == true) {
                win_count += 1;
            }
    ...
    if (count != 0)
        return floatDiv(win_count, count) + " = " + win_count + "(win) / " + count + "(count)";
    else
        return 0;
}
function deal_teams_data_json(json_data) {
    teams_data = [];
    //对数据处理，得到想要的teams_data
    for (let index = 1; index < hero_id_name.length; index++) {
        var element = {};
        element['hero_id'] = index;
        element['hero_name'] = hero_id_name[index];
        element['og'] = calculate(json_data["og"],index);
        element['liquid'] = calculate(json_data["liquid"],index);
        element['lgd'] = calculate(json_data["lgd"],index);
        element['secret'] = calculate(json_data["secret"],index);
        element['vg'] = calculate(json_data["vg"],index);
        element['eg'] = calculate(json_data["eg"],index);
        teams_data[index - 1] = element;
    }
}
...
layui.use(['laydate', 'laypage', 'layer', 'table', 'carousel', 'upload', 'element'], function () {
    ...
    var creattable = table.render({
        ...
        , limit: 113//显示的数量
        , cols: [[
            { field: 'hero_id', title: '英雄_id', sort: true }
            , { field: 'hero_name', title: '英雄名称', minWidth: 160 }
            , { field: 'og', title: 'OG', sort: true }
            , { field: 'liquid', title: 'Team Liquid', sort: true }
            , { field: 'lgd', title: 'PSG.LGD', sort: true }
            , { field: 'secret', title: 'Team Secret', sort: true }
            , { field: 'vg', title: 'Vici Gaming', sort: true }
            , { field: 'eg', title: 'Evil Geniuses', sort: true }
        ]]
        , data: teams_data
    });
//如果想要定制化数据，可以通过python自己写处理代码，获得数据，然后上传即可
    var uploadInst = upload.render({
        ...
        , done: function (res) {
            ...
            //上传成功
            layer.msg('读取成功');
            deal_teams_data_json(JSON.parse(res.data));
            creattable.reload({
                elem: '#date_table'
                ,cellMinWidth: 60 //全局定义常规单元格的最小宽度，layui 2.2.1 新增
                ,cols: [[
                    {field:'hero_id', title: '英雄_id', sort: true}
                    ,{field:'hero_name', title: '英雄名称'}
                    ,{field:'og', title: 'OG', sort: true}
                    ,{field:'liquid', title: 'Team Liquid', sort: true}
                    ,{field:'lgd', title: 'PSG.LGD', sort: true}
                    ,{field:'secret', title: 'Team Secret', sort: true}
